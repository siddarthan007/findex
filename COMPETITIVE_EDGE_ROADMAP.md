...
| P0 | Intent router with confidence/stop policy | Relationship parsing, bounded concept expansion, multi-leg fusion, and revision-safe paraphrase caching now ship; automatic leg/stop confidence still needs evaluation. | Use scores, index availability, duplicate rate, and graph evidence to select legs and stop. | Fewer calls/tokens with no Recall@10 loss. |
...
| P1 | Hierarchical code communities | Deterministic module hierarchy, weak components, hubs, and source-free summaries now ship as a bounded baseline; large repositories still need stronger partitions. | Add incremental Leiden/label-propagation communities and drill-down retrieval to exact ranges. | Better global-question recall under a fixed token budget. |
